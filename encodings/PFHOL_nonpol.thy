theory PFHOL_nonpol imports Main
begin 

(* A nonpolymorphic Embedding of Positive Free Higher-Order Logic (PFHOL) in HOL *)
...
text \<open> Positive Free Higher-Order Logic \<close>
  ...
  consts fUndefI :: "i" ("\<^bold>e\<^sup>i") (* Distinguished symbol for undefinedness among the individuals *)
  ...
  *)
  ...
  definition fOr (infixr "\<^bold>\<or>" 51) (* — Free disjunction *)
  ...
  named_theorems Defs declare fIdentityI_def[Defs] fNot_def[Defs] fOr_def[Defs] fForallI_def[Defs] 
    ...
    fThatIBinder_def[Defs] fAnd_def[Defs] fImp_def[Defs] fEquiv_def[Defs] fExistsI_def[Defs] 
    fExistsIBinder_def[Defs] fExistsP_def[Defs] fExistsPBinder_def[Defs]


text \<open> Some Tests \<close>

  lemma "(\<^bold>\<forall>\<^sup>ix. P x) \<^bold>\<rightarrow>  P x" nitpick [user_axioms=true, show_all, format=2] oops (* properly invalid *)
  lemma "((\<^bold>\<forall>\<^sup>ix. (P x)) \<^bold>\<and> (E\<^sup>i x)) \<^bold>\<rightarrow> (P x)" unfolding Defs by auto (* properly valid *)

  lemma "P x \<^bold>\<rightarrow> (\<^bold>\<exists>\<^sup>ix. P x)" nitpick [user_axioms=true, show_all, format=2] oops (* properly invalid *)
  lemma "((P x) \<^bold>\<and> (E\<^sup>i x)) \<^bold>\<rightarrow> (\<^bold>\<exists>\<^sup>ix. P x)" unfolding Defs by auto (* properly valid *)
  
  lemma "(P x) \<^bold>\<and> (\<^bold>\<exists>\<^sup>ix. P x)" nitpick [user_axioms=true, show_all, format=2] oops (* properly invalid *)
  lemma "(P x) \<^bold>\<rightarrow> (E\<^sup>i x)" nitpick [user_axioms=true, show_all, format=2] oops (* properly invalid *)
  ...
  lemma "(P i\<^sub>1)" nitpick [user_axioms=true, show_all, format=2] oops (* properly invalid *)
  lemma "\<^bold>\<not>(P i\<^sub>1)" nitpick [user_axioms=true, show_all, format=2] oops (* properly invalid *)
...
text \<open> Prior's Theorem \<close>
  ...
  lemma "(P (\<forall>p. (P p \<longrightarrow> (\<not>p)))) \<longrightarrow> ((\<exists>p. P p \<and> p) \<and> (\<exists>p. P p \<and> (\<not>p)))" by blast
  ...
  lemma "(Q (\<^bold>\<forall>\<^sup>bp. (Q p \<^bold>\<rightarrow> (\<^bold>\<not>p)))) \<^bold>\<rightarrow> ((\<^bold>\<exists>\<^sup>bp. Q p \<^bold>\<and> p) \<^bold>\<and> (\<^bold>\<exists>\<^sup>bp. Q p \<^bold>\<and> (\<^bold>\<not>p)))" 
  ...
  lemma "(Q (\<^bold>\<forall>\<^sup>bp. (Q p \<^bold>\<rightarrow> (\<^bold>\<not>p)))) \<^bold>\<rightarrow> ((\<^bold>\<exists>\<^sup>bp. Q p \<^bold>\<and> p) \<^bold>\<and> (\<^bold>\<exists>\<^sup>bp. Q p \<^bold>\<and> (\<^bold>\<not>p)))" 
    unfolding Defs
    by (smt fExistsBBinder_def fExistsB_def fFalseAxiom fForallBBinder_def fForallB_def fNot_def fTrueAxiom)
